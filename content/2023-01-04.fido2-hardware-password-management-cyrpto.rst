...
- Hardware token/device returns ``HMAC(salt, secret-generated-on-device)``,
...
that ideally won't be used for anything else with that auth token, which is
itself not a secret of any kind.

This works well between different devices with authenticators that support FIDO2
Discoverable Credentials (aka Resident Keys), to have HMAC key stored on the
device, but for simplier tokens that don't support that, static credential-id
value (returned by `fido2-cred tool`_) also needs to be built-in via -DFHD_CID=
compile-time option (and is also not a secret).

  (technically that last "credential id" value has device-master-key-wrapped
  HMAC key, but it's only possible to get it from there by the device itself,
  and it's never passed or exposed anywhere in plaintext at any point)
...
I.e. what if FIDO2 token in question breaks or gets lost?
...
secret-block to it, before encrypting all secrets in that block for good.
