...
NFC from a phone (quick demo like `this 30s youtube video`_ might help to get the idea).
  ...
  - Click "Accept" on the phone, run nfc-epaper-writer_ app and "Load Image" there.
...
Big upside of using eink pad for this is that whatever info/reminder always stays
there to check anytime (incl. hours, days or months later), without needing
free/clean hands to drop everything and tinker with small phone screen,
which has to be clumsily tapped/scrolled, goes to sleep, discharges and dies,
...
Which tbh has its own charm with dithering_ for images (see didder_ wrapper at the end),
