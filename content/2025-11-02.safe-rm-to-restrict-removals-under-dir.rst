...
- Run ``fd = openat2(p, RESOLVE_NO_SYMLINKS)`` to open that path (with optional
  ...
  or treat that as a special "file vanished" error.
...
(like for a `fuse-filesystem overlay like acfs`_, where fixing every access to be
...
.. _fuse-filesystem overlay like acfs:
