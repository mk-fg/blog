...
If case something doesn't work in such a rosy scenario, which kinda happens
often, first place to look at is probably debug output of bluetoothd itself, which
...
"Connection refused - security block" error - no idea why BlueZ throws it in
this context and google doesn't seem to help much.

Didn't need these modes though, and I guess it might be an hw issue too - just
missing support in the firmware, if stuff on this level needs to be handled
there.
