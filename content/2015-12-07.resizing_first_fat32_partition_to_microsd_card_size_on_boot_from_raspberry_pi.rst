...
it's /boot FAT32 partition to full card size (i.e. "make it as large as
possible") from right underneath itself.

RPis have first FAT (fat16 / fat32 / vfat) partition needed by firmware to load
config.txt and uboot stuff off, and that is the only partition one can see in
Windows OS when plugging microSD card into card-reader, which is kinda arbitrary
OS limitation.
